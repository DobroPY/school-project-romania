...
const StudentGrades = (props) => {
    ...
    const firstPage= subjectPages[0].name
  ...
  return (
    ...
    <div className="mx-[2%] mt-4">
        ...
        {subjectPages.map((sub, index)=>{
            return(
                <div>
                <p className={currentPage == sub.name ? "px-6 py-2 border-b border-purple-600 bg-purple-100 text-purple-600 cursor-pointer" : "cursor-pointer px-6 py-2"} onClick={()=>{ changePage(sub.name)}}>{sub.name}</p>
                </div>
                );
        })}
        </div>
