...
import { getStudents } from "../apis/get";
...
const Students = () => {
  ...
  // crate select of classrooms

  const [students, setStudents]: any = useState([]);
  const [classSelect, setClassSelect]: any = useState([]);
  const [allStudents, setAllStudent]: any = useState([]);
  ...
  const getStudents = async () => {
    ...
    for (let i = 0; i < studentsData.length; i++) {
      ...
      for (let j = 0; j < absences.length; j++) {
        if (studentsData[i].id == absences[j].student) {
          sum += 1;
        }
        studentsData[i].absences = sum;
      }
    }
    ...
    for (let i = 0; i < studentsData.length; i++) {
      ...
      for (let j = 0; j < grades.length; j++) {
        if (studentsData[i].id == grades[j].student) {
          gradesSum += grades[j].grade;
          rewardsSum += grades[j].reward;
          count += 1;
        }
        if (count > 0) {
          studentsData[i].gradesAverage = gradesSum / count;
          studentsData[i].rewardsAverage = rewardsSum / count;
        } else {
          studentsData[i].gradesAverage = 0;
          studentsData[i].rewardsAverage = 0;
        }
      }
    }
  ...
  const selectClass = (event) => {
    ...
    const allData = allStudents;

    const newStudents = allData.filter((item) => item.class == select);
  ...
  return (
    <section className="w-4/5">
      ...
      <div className="ml-6 mt-10  flex justify-between">
        ...
        <CreateStudentModal modules={modules} />
      </div>
              ...
              <tr className="text-left" key={index}>
                <td className="p-2 flex">
                  ...
                  <div className="username ml-2 text-left">
                    ...
                    <p className="text-sm font-light">ID: {student.id}</p>
                  </div>
                </td>
